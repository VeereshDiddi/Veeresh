...
-(void)LaunchView
{
    ...
    if (UI_USER_INTERFACE_IDIOM() == UIUserInterfaceIdiomPad)
    {
        ...
        //NSLog(@"not ipad, device width: %f  height: %f", result1.width, result1.height);
...
-(void)octoshapeStreamCalled:(NSString *)octolink
{
    ...
    self.streamPlayer = [self.octoshapeSystem createStreamPlayer:octolink];
...
-(void)viewDidDisappear:(BOOL)animated {
    NSLog(@"VideoPlayer:viewDidDisappear() ...");
