  ...
  sourceApplication:(NSString *)sourceApplication annotation:(id)annotation
{
    ...
    if (webInfo.length > 0) {
        ...
        if (urlLink == nil)
            urlLink = [[NSString alloc] init];
        
        urlLink = [parameters objectForKey:@"octo_url"];
        
        if (webUrl == nil)
            webUrl = [[NSString alloc] init];
        
        webUrl = [parameters objectForKey:@"page_url"];
        //NSLog (@"webpage: %@", webUrl);
        //NSString *local_link = [parameters objectForKey:@"url_link"];
        //NSLog(@"parameters: %@ %@", parameters, urlLink);
    }
